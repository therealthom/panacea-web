...
<div class="control-group fieldcontain ${hasErrors(bean: promocionDeVersionInstance, field: 'proyecto', 'error')} required">
    ...
    <div class="controls">
        <g:select size="3" id="proyecto" name="proyecto.id" from="${com.web.panacea.ConfiguracionDeProyecto.list()}" optionKey="id" required="" value="${promocionDeVersionInstance?.proyecto?.id}" class="many-to-one"/>
    </div>
</div>
...
<div class="control-group fieldcontain ${hasErrors(bean: promocionDeVersionInstance, field: 'ambiente', 'error')} required">
    ...
    <div class="controls">
        <g:select size="3" name="ambiente" from="${promocionDeVersionInstance.constraints.ambiente.inList}" required="" value="${promocionDeVersionInstance?.ambiente}" valueMessagePrefix="promocionDeVersion.ambiente"/>
    </div>
</div>
    ...
    <div class="controls">
        ...
        <ul class="one-to-many">
            <g:each in="${promocionDeVersionInstance?.documentos?}" var="d">
                <li><g:link controller="documentacionDeProyecto" action="show" id="${d.id}">${d?.encodeAsHTML()}</g:link></li>
                </g:each>
            <li class="add">
